...
- Every LLM import is written in English whatever language the source is in - the rule lives in `DRAFT_FIELD_RULES` in `src/server/import/llm/provider.ts`, which every system prompt includes, and `provider.test.ts` pins it. The JSON-LD path never touches a model, so a non-English structured recipe is copied as written and only flagged with a warning (from schema.org `inLanguage`).
